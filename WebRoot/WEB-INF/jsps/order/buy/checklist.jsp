...
<script type="text/javascript">
	...
	function viewAduitOrder(orderId){
		var diag = new Dialog();
		diag.Width = 450;
		diag.Height = 201;
		diag.ShowButtonRow=true;
		diag.Title = "审核意见";
		diag.URL = "${path}/orderModel_aduitText";
		diag.OKEvent = function(){
			var win = diag.innerFrame.contentWindow;
			//调用提交表单的审核意见
			var note = win.getNote();
			var result = aduitOrder(orderId,2,note);
		...
		diag.addButton("next","驳回",function(){
			...
			//调用提交表单的审核意见
			var note = win.getNote();
			var result = aduitOrder(orderId,3,note);
			//alert(result)
			if(result == "success"){
				diag.close();
				window.location.href = "${path }/orderModel_checklist?query.orderType=1&query.orderState=1";
	...
	function aduitOrder(orderId,orderState,note){
		var result = "";
		$.ajax({
			url:"${path}/ajax_orderModel_aduitOrder",
			type:"post",
			data:{
				...
				<table width="100%" border="1" cellpadding="0" cellspacing="0">
					...
					<s:iterator value="#page.list" var="order">
						<tr align="center" bgcolor="#FFFFFF">
							...
							<td align="center"><s:property value="#order.totalPrice"/> 元</td>
							<td>
								<input type="button" onclick="viewDetail(<s:property value="#order.orderId"/>)" value="详情">
								<s:if test="#order.orderState == 1">
									<input type="button" onclick="viewAduitOrder(<s:property value="#order.orderId"/>)" value="审核">
								</s:if>
							</td>
							<!-- <td>未审核</td> -->
						</tr>
